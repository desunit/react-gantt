...
import {
  prepareEditTask as $t,
  grid as Kt,
  extendDragOptions as Ot,
  isSegmentMoveAllowed as Vt,
  DataStore as qt,
  normalizeLinks as Bt,
  normalizeZoom as jt,
  defaultColumns as Qt,
  parseTaskDates as gt,
  defaultTaskTypes as Ut,
  getToolbarButtons as xt,
  handleAction as Mt,
  isHandledAction as Ct,
  getMenuOptions as wt,
  getEditorItems as Zt,
} from '@svar-ui/gantt-store';
import {
  defaultColumns as ns,
  defaultEditorItems as ss,
  defaultMenuOptions as rs,
  defaultTaskTypes as os,
  defaultToolbarButtons as is,
  getEditorItems as ls,
  getMenuOptions as cs,
  getToolbarButtons as as,
  registerScaleUnit as ds,
} from '@svar-ui/gantt-store';
import {
  useWritableProp as rt,
  useStore as W,
  useStoreWithCounter as Ue,
  writable as Jt,
  useStoreLater as Pe,
...
function ln(t, n) {
  ...
  function H(p) {
    ...
    if (!l) {
      if (
        (Math.abs(F) < kt && Math.abs(Z) < kt) ||
        (n && n.start && n.start({ id: o, e: p }) === !1)
      )
        return;
      (l = s.cloneNode(!0)),
        (l.style.pointerEvents = 'none'),
        l.classList.add('wx-reorder-task'),
        (l.style.position = 'absolute'),
        (l.style.left = w.left + 'px'),
        (l.style.top = w.top + 'px'),
        (s.style.visibility = 'hidden'),
        s.parentNode.insertBefore(l, s);
    }
    if (l) {
      const re = Math.round(Math.max(0, w.top + Z));
      if (n && n.move && n.move({ id: o, top: re, detail: x }) === !1) return;
      const O = n.getTask(o),
        te = O.$y;
      if (!w.start && w.y == te) return G();
      (w.start = !0), (w.y = O.$y - 4), (l.style.top = re + 'px');
      const E = document.elementFromPoint(p.clientX, p.clientY),
        M = ze(E);
      if (M && M !== s) {
        const g = Ye(M),
          X = M.getBoundingClientRect(),
          B = X.top + X.height / 2,
          J = p.clientY + w.db > B && M.nextElementSibling !== s,
          ce = p.clientY - w.dt < B && M.previousElementSibling !== s;
        x?.after == g || x?.before == g
          ? (x = null)
          : J
            ? (x = { id: o, after: g })
            : ce && (x = { id: o, before: g });
  ...
  function z(p) {
    y
      ? (p.preventDefault(), H(p.touches[0]))
  ...
  return /* @__PURE__ */ Re('div', {
    ...
    children: [
      ...
      /* @__PURE__ */ a('div', {
        ...
        children: l ? /* @__PURE__ */ a(l, { row: t, column: n }) : t.text,
      }),
    ],
  });
}
...
function an(t) {
  const {
    ...
    te = W(c, 'splitTasks'),
    [E, M] = ue(null),
    g = b(() => (!G || !k ? [] : G.slice(k.start, k.end)), [G, k]),
    X = $(
      (i, m) => {
        if (m === 'add-task')
          c.exec(m, {
    ...
    B = $(
      (i) => {
        const m = Fe(i),
          v = i.target.dataset.action;
        v && i.preventDefault(),
          m
            ? v === 'add-task' || v === 'open-task'
              ? X(m, v)
              : c.exec('select-task', {
                  id: m,
  ...
  const oe = ie(null),
    ...
    R = b(() => (E && !g.find((i) => i.id === E.id) ? [...g, E] : g), [g, E]),
    ee = b(() => {
      if (!w || !x) return R;
    ...
    ge = b(() => {
      ...
      const m = i.findIndex((K) => K.id === 'text'),
        v = i.findIndex((K) => K.id === 'add-task');
      if (
        (m !== -1 && (i[m].cell && (i[m]._cell = i[m].cell), (i[m].cell = cn)),
        v !== -1)
      ) {
        i[v].cell = i[v].cell || bt;
        const K = i[v].header;
        if (
          (typeof K != 'object' && (i[v].header = { text: K }),
          (i[v].header.cell = K.cell || bt),
          n)
        )
          i.splice(v, 1);
        else if (s) {
    ...
    Ce = $(() => ge.some((i) => i.flexgrow && !i.hidden), []),
    Le = b(() => Ce(), [Ce, u]),
    Ne = b(() => {
      ...
      if (!Le) {
        ...
        if (ge.some((xe) => xe.$width)) {
          ...
          (v = ge.reduce(
            (me, Te) => (
              Te.hidden || ((xe += Te.width), (me += Te.$width || Te.width)), me
            ),
            0,
          )),
            xe > v && v > m && (K = !0);
    ...
    fe = $(
      (i) => {
        if (!Ce()) {
          const m = Ne.reduce(
            (v, K) => (
              i && K.$width && (K.$width = K.width),
              v + (K.hidden ? 0 : K.width)
            ),
            0,
          );
          m !== y && d(m);
    ...
    f = $(() => {
      ge.filter((m) => m.flexgrow && !m.hidden).length === 1 &&
        ge.forEach((m) => {
    ...
    Q = $(
      (i) => {
        if (!n) {
          const m = Fe(i),
            v = Xt(i, 'data-col-id');
          !(v && ge.find((xe) => xe.id == v))?.editor &&
            m &&
            c.exec('show-editor', { id: m });
        }
      },
      [c, n],
      // cols is defined later; relies on latest value at call time
    ),
    he = b(() => (Array.isArray(q) ? q.map((i) => i.id) : []), [q]),
    N = $(() => {
  ...
  qe(),
    se(() => {
      qe();
    }, [D, Xe, Z, c, f, fe, g, re, O, te, r]);
  const tt = $((i) => {
    D(i),
      i.intercept('hotkey', (m) => Se.current.handleHotkey(m)),
      i.intercept('scroll', () => !1),
      i.intercept('select-row', () => !1),
      i.intercept('sort-rows', (m) => {
        const v = Se.current.sortVal,
          { key: K, add: xe } = m,
          me = v ? v.find((De) => De.key === K) : null;
        let Te = 'asc';
        return (
          me && (Te = !me || me.order === 'asc' ? 'desc' : 'asc'),
          c.exec('sort-tasks', {
            key: K,
            order: Te,
            add: xe,
      ...
      i.intercept('update-cell', (m) => {
        const { id: v, column: K, value: xe } = m,
          me = Se.current.tasks.find((Te) => Te.id === v);
        if (me) {
          const Te = { ...me };
          let De = xe;
          De && !isNaN(De) && !(De instanceof Date) && (De *= 1),
            (Te[K] = De),
            $t(
              Te,
              {
...
function fn(t) {
  ...
  function o(S) {
    ...
    const r = (n.duration * n.progress) / 100,
      w = n.segments;
    let x = 0,
      y = 0,
      d = null;
    do {
      const L = w[y];
      y === S &&
        (x > r ? (d = 0) : (d = Math.min((r - x) / L.duration, 1) * 100)),
        (x += L.duration),
        y++;
...
function hn(t) {
  const {
    ...
    re = b(() => D.lengthUnitWidth, [D]),
    O = b(() => D.lengthUnit || 'day', [D]),
    te = ie(!1),
    [E, M] = ue(void 0),
    [g, X] = ue(null),
    B = ie(null),
    ...
    ee = b(() => R && c[c.length - 1]?.id, [R, c]);
  se(() => {
    if (V && R && V) {
      const { id: e } = V,
        h = ye.current?.querySelector(`.wx-bar[data-id='${e}']`);
  ...
  const ge = $(() => {
    ...
    $e = $(
      (e, h, T) => {
        if (
          ...
          (T || (T = r.getTask(Ye(e))),
          T.type === 'milestone' || T.type === 'summary')
        )
          return '';
        const I = ze(h, 'data-segment');
        I && (e = I);
        const { left: A, width: ne } = e.getBoundingClientRect(),
          de = (h.clientX - A) / ne;
    ...
    Le = $(
      (e) => {
        const h = ye.current;
        if (!h) return [];
        const T = h.parentElement?.scrollLeft || 0,
          I = h.parentElement?.parentElement?.scrollTop || 0,
          A = Math.min(e.startX, e.currentX),
          ne = Math.max(e.startX, e.currentX),
          de = Math.min(e.startY, e.currentY),
          ke = Math.max(e.startY, e.currentY);
        return w.filter((we) => {
          const Ee = we.$x - T,
            He = we.$x + we.$w - T,
            _e = (Ce.get(we.id) ?? we.$y) - I,
            Me = _e + we.$h;
    ...
    pe = $(() => {
      if (_) {
        ...
        _.ctrlKey
          ...
          : (c.length > 0 && r.exec('select-task', { id: null, marquee: !0 }),
            e.forEach((h, T) => {
              r.exec('select-task', {
                id: h.id,
                toggle: T > 0,
      ...
      if (U) {
        const { dx: e, originalPositions: h } = U,
          T = Math.round(e / re);
        if (T !== 0) {
          let I = !0;
          h.forEach((A, ne) => {
      ...
      if (B.current) {
        ...
        (B.current = null),
          ...
          T.classList.remove('wx-progress-in-drag'),
          (te.current = !0),
          ve();
      } else if (g) {
        const {
          ...
          if (!we)
            ...
            const Be = 1440 * 60 * 1e3,
              Me =
                ...
                Be;
            h === 'move'
              ? ((Ee.start = new Date(He.start.getTime() + Me)),
                (Ee.end = new Date(He.end.getTime() + Me)))
              : h === 'start'
                ? ((Ee.start = new Date(He.start.getTime() + Me)),
                  (Ee.end = He.end))
                : h === 'end' &&
    ...
    Ie = $(
      (e, h) => {
        ...
        if (!n) {
          ...
          if (B.current) {
            const { node: A, x: ne, id: de } = B.current,
              ke = (B.current.dx = T - ne),
              we = Math.round((ke / A.offsetWidth) * 100);
            let Ee = B.current.progress + we;
            (B.current.value = Ee = Math.min(Math.max(0, Ee), 100)),
              r.exec('update-task', {
                id: de,
                task: { progress: Ee },
                inProgress: !0,
            ...
            const A = ze(e);
            if (A) {
              const ne = r.getTask(Ye(A)),
                ke = ze(e, 'data-segment') || A,
                we = $e(ke, h, ne);
    ...
    Xe = $(
      (e) => {
        le
          ? (e.preventDefault(), Ie(e, e.touches[0]))
  ...
  const tt = $(
    ...
    v = $(
      (e, h) => {
        ...
        return e === T
          ? !0
          : !!y.find(
              (A) => A.target == e && A.source == T && A.type === m(I, h),
            );
      },
      [E, d, m],
    ),
    ...
    De = $(
      (e) => {
        if (le || u.current) return e.preventDefault(), !1;
      },
      [le],
    ),
    it = b(() => C.map((e) => e.id), [C]),
    lt = $(
      (e) => {
        ...
        return (
          ['task', 'milestone', 'summary'].includes(e) || (h = `task ${h}`), h
        );
      },
      [it],
    ),
    ...
    nt = $((e) => z && q.byId(e).$critical, [z, q]),
    at = $(
      (e) => {
        if (k?.auto) {
          const h = q.getSummaryId(e, !0),
            T = q.getSummaryId(E.id, !0);
          return (
            E?.id &&
            !(Array.isArray(h) ? h : [h]).includes(E.id) &&
    ...
    dt = b(() => {
      if (!_) return null;
      const e = Math.min(_.startX, _.currentX),
        h = Math.min(_.startY, _.currentY),
        T = Math.abs(_.currentX - _.startX),
        I = Math.abs(_.currentY - _.startY);
      return {
        left: `${e}px`,
        top: `${h}px`,
        width: `${T}px`,
  ...
  return /* @__PURE__ */ Re('div', {
    ...
    children: [
      ...
      Z.map((e) => {
        ...
        const h =
            `wx-bar wx-${lt(e.type)}` +
            (le && g && e.id === g.id ? ' wx-touch' : '') +
            (E && E.id === e.id ? ' wx-selected' : '') +
            (Ne.has(e.id) ? ' wx-selected' : '') +
            (nt(e.id) ? ' wx-critical' : '') +
            (e.$reorder ? ' wx-reorder-task' : '') +
            (G && e.segments ? ' wx-split' : ''),
          T =
            'wx-link wx-left' +
            (E ? ' wx-visible' : '') +
            (!E || (!v(e.id, !0) && at(e.id)) ? ' wx-target' : '') +
            (E && E.id === e.id && E.start ? ' wx-selected' : '') +
            (nt(e.id) ? ' wx-critical' : ''),
          I =
            'wx-link wx-right' +
            (E ? ' wx-visible' : '') +
            (!E || (!v(e.id, !1) && at(e.id)) ? ' wx-target' : '') +
            (E && E.id === e.id && !E.start ? ' wx-selected' : '') +
            (nt(e.id) ? ' wx-critical' : '');
        return /* @__PURE__ */ Re(
          Lt,
...
function wn(t) {
  ...
  function g() {
    const u = F.current,
      oe = Math.ceil((G || 0) / (Y || 1)) + 1,
      _ = Math.floor(((u && u.scrollTop) || 0) / (Y || 1)),
      j = Math.max(0, _ - Z),
      U = _ + oe + Z,
      be = j * (Y || 0);
    d.exec('render-data', {
  ...
  const X = $(
    (u) => {
      ...
      const _ = F.current;
      if (!_) return;
      const { clientWidth: j } = _,
        U = d.getTask(P);
      if (U.$x + U.$w < _.scrollLeft)
        d.exec('scroll-chart', { left: U.$x - (c || 0) }),
          (_.scrollLeft = U.$x - (c || 0));
      else if (U.$x >= j + _.scrollLeft) {
        const be = j < U.$w ? c || 0 : U.$w;
        d.exec('scroll-chart', { left: U.$x - j + be }),
          (_.scrollLeft = U.$x - j + be);
      }
    },
    [d, c],
  );
  ...
  function B(u) {
    if (q && (u.ctrlKey || u.metaKey)) {
      u.preventDefault();
      const P = F.current,
        oe = -Math.sign(u.deltaY),
        _ = u.clientX - (P ? P.getBoundingClientRect().left : 0);
      d.exec('zoom-scale', {
        dir: oe,
        offset: _,
      });
  ...
  const ce = b(
      () =>
        H && (H.minUnit === 'hour' || H.minUnit === 'day') && r
          ? H.rows[H.rows.length - 1].cells.map(J)
          : null,
      [H, r],
    ),
    le = $(
...
function pn(t) {
  ...
  function Y(ce) {
    ...
    if (l != 'x') for (let u in ae) ae[u] = ae[u].reverse();
    return ae;
  }
  const [c, H] = ue(!1),
    [V, z] = ue(null),
  ...
  const Z = $(
      (ce) => {
        ...
        L(le);
        let ae;
        le <= x ? (ae = 'chart') : w - le <= y ? (ae = 'grid') : (ae = 'all'),
          p.current !== ae && (C(ae), (p.current = ae)),
          G.current && clearTimeout(G.current),
          (G.current = setTimeout(() => o && o(le), 100));
      },
      [w, x, y, o],
    ),
    re = $(() => {
  ...
  function M(ce) {
    if (r) {
      ...
      C(le), S(le);
    } else if (D === 'grid' || D === 'chart') E(), S('all');
    else {
...
function kn(t) {
      ...
      (R) => {
        V(
          (ee) => (
            R !== ee &&
              (R
                ? ((E.current = O), O === 'all' && te('grid'))
                : (!E.current || E.current === 'all') && te('all')),
            R
          ),
        );
      },
  ...
  const g = b(() => {
    let R;
    return (
      C.every((ee) => ee.width && !ee.flexgrow)
        ? (R = C.reduce((ee, ge) => ee + parseInt(ge.width), 0))
        : H && O === 'chart'
          ? (R = parseInt(C.find((ee) => ee.id === 'action')?.width) || 50)
          : (R = 440),
      (z = R),
      R
    );
  ...
  const X = b(() => (k ?? 0) - (Z ?? 0), [k, Z]),
    B = b(() => L.width, [L]),
    J = b(() => {
      if (!r || !w) return d.length * D;
  ...
  se(() => {
    let R;
    return (
      ae.current && ((R = new ResizeObserver(u)), R.observe(ae.current)),
      () => {
        R && R.disconnect();
      }
    );
  ...
  const U = $(
    (R) => {
      ...
      if (!Ce) return;
      const { id: Ne } = R,
        fe = ee.findIndex((f) => f.id === Ne);
      if (fe > -1) {
        const f = Le - ge.height,
          Q = fe * ve,
          he = Ce.scrollTop;
        let N = null;
        Q < he ? (N = Q) : Q + ve > he + f && (N = Q - f + ve + $e),
          N !== null &&
            (y.exec('scroll-chart', { top: Math.max(N, 0) }),
            (P.current.scrollTop = Math.max(N, 0)));
      }
    },
    [y],
  );
...
const Cn = (t) =>
  ...
  On = vt(function (
    ...
    const u = b(() => new qt(Jt), []),
      P = b(() => ({ ...ot, ...et }), []),
      oe = Ae(Ge.i18n),
      _ = b(() => (oe ? oe.extend(P, !0) : Je(P)), [oe, P]),
      j = b(() => _.getRaw().calendar, [_]),
      U = b(() => {
        let fe = {
          zoom: Mn(q, j),
          scales: Nt(x, j),
        ...
        return (
          fe.zoom &&
            (fe = {
              ...fe,
              ...jt(fe.zoom, Tn(j, _.getGroup('gantt')), fe.scales, Y),
            }),
          fe
        );
      }, [q, x, y, w, Y, j, _]),
          ...
          Q = (he) => {
            he.forEach((N) => {
              ...
              f.set(N.id, pe),
                fe.has(pe) || fe.set(pe, []),
                fe.get(pe).push(N.id),
                N.data && N.data.length > 0 && Q(N.data);
            });
          };
    ...
    const Ce = b(
      () => ({
        ...
        getTable: (fe) =>
          fe
            ? new Promise((f) => setTimeout(() => f($e.current), 1))
            : $e.current,
    ...
    const Ne = b(
      () =>
        M
          ? (fe, f) =>
              (f == 'day' && !M.getDayHours(fe)) ||
              (f == 'hour' && !M.getDayHours(fe))
                ? 'wx-weekend'
                : ''
          : G,
      [M, G],
    );
    return /* @__PURE__ */ a(Ge.i18n.Provider, {
      value: _,
...
function Vn({ api: t = null, items: n = [] }) {
  const s = Ae(Ge.i18n),
    ...
    d = b(() => {
      ...
      return (
        ...
        let c = { ...Y, disabled: !1 };
        return (
          (c.handler = Ct(D, c.id) ? (H) => Mt(t, H.id, null, o) : c.handler),
        ...
        d.forEach((C) => {
          ...
          if (Y === 'add-task') D.push(C);
            ...
            D.push({
              ...C,
              disabled:
                C.isDisabled && S.some((c) => C.isDisabled(c, t.getState())),
            });
          }
        }),
        D.filter((C, Y) => {
  ...
  return s
    ? /* @__PURE__ */ a(pt, { items: L })
    : /* @__PURE__ */ a(Ge.i18n.Provider, {
        value: l,
        children: /* @__PURE__ */ a(pt, { items: L }),
      });
}
const qn = vt(function (
  ...
  function k(M) {
    return M.map(
      (g) => (
        (g = { ...g }),
        g.text && (g.text = Y(g.text)),
        g.subtext && (g.subtext = Y(g.subtext)),
        g.data && (g.data = k(g.data)),
        g
      ),
    );
  }
  function G() {
    const M = n.length ? n : wt({ splitTasks: z }),
      g = M.find((X) => X.id === 'convert-task');
    return (
      g &&
        ((g.data = []),
        (c || []).forEach((X) => {
    ...
    Z = $(
      (M, g) => {
        ...
        if (X) {
          ...
          B !== null
            ? (L.current = { id: X.id, segmentIndex: B })
            : (L.current = X.id),
            (!Array.isArray(H) || !H.includes(X.id)) &&
              s &&
              s.exec &&
              s.exec('select-task', { id: X.id });
        }
        return X;
      },
      [s, l, H],
    ...
    O = $(
      (M, g) => {
        ...
        if (
          B &&
          (M.isHidden &&
            (B = !X.some((J) => M.isHidden(J, s.getState(), L.current))),
          M.isDisabled)
        ) {
          const J = X.some((ce) => M.isDisabled(ce, s.getState(), L.current));
          M.disabled = J;
        }
        return B;
      },
...
function En({ api: t, autoSave: n, onLinksChange: s }) {
  const o = Ae(Ge.i18n).getGroup('gantt'),
    S = W(t, 'activeTask'),
    r = W(t, '_activeTask'),
    w = W(t, '_links'),
    x = W(t, 'schedule'),
    y = W(t, 'unscheduledTasks'),
    [d, L] = ue();
  function D() {
    if (S) {
      const H = w
          .filter((z) => z.target == S)
          .map((z) => ({ link: z, task: t.getTask(z.source) })),
        V = w
          .filter((z) => z.source == S)
          .map((z) => ({ link: z, task: t.getTask(z.target) }));
      return [
                          ...
                          children: [
                            /* @__PURE__ */ a('td', {
                              ...
                              children: /* @__PURE__ */ a('div', {
                                ...
                                children: z.task.text || '',
                              }),
                            }),
                            x?.auto && z.link.type === 'e2s'
                              ? /* @__PURE__ */ a('td', {
...
function Nn(t) {
  const { value: n, time: s, format: l, onchange: o, onChange: S, ...r } = t,
    w = S ?? o;
  function x(y) {
...
function Bn({
  ...
  const G = W(t, '_activeTask'),
    p = W(t, 'activeTask'),
    F = W(t, 'unscheduledTasks'),
    Z = W(t, 'links'),
    re = W(t, 'splitTasks'),
    O = b(() => re && p?.segmentIndex, [re, p]),
    te = b(() => O || O === 0, [O]),
    E = b(() => Zt({ unscheduledTasks: F }), [F]),
    M = W(t, 'undo'),
    [g, X] = ue({}),
    [B, J] = ue(null),
    [ce, le] = ue(),
    ...
    oe = b(() => {
      ...
      if ((te && G.segments ? (f = { ...G.segments[O] }) : (f = { ...G }), o)) {
        ...
        return (
          E.forEach(({ key: he, comp: N }) => {
            if (N !== 'links') {
              const pe = f[he];
              N === 'date' && pe instanceof Date
                ? (Q[he] = c(pe))
                : N === 'slider' && he === 'progress'
                  ? (Q[he] = `${pe}%`)
                  : (Q[he] = pe);
            }
          }),
  ...
  function _(f, Q) {
    return f.map((he) => {
      const N = { ...he };
      if (
        (he.config && (N.config = { ...N.config }),
        N.comp === 'links' &&
          t &&
          ((N.api = t), (N.autoSave = x), (N.onLinksChange = be)),
      ...
      return (
        N.comp === 'slider' &&
          N.key === 'progress' &&
          (N.labelTemplate = (pe) => `${C(N.label)} ${pe}%`),
        N.label && (N.label = C(N.label)),
        N.config?.placeholder &&
          (N.config.placeholder = C(N.config.placeholder)),
        Q &&
          (N.isDisabled && N.isDisabled(Q, t.getState())
            ? (N.disabled = !0)
            : delete N.disabled),
        N
      );
  ...
  const j = b(() => {
      ...
      return (
        (f = _(f, ce)),
        ce ? f.filter((Q) => !Q.isHidden || !Q.isHidden(ce, t.getState())) : f
      );
    }, [n, E, ce, P, C, t, x]),
    U = b(() => j.map((f) => f.key), [j]);
  ...
  const ye = $(() => {
    ...
    R = $(() => {
      ...
      if (te) {
        if (G?.segments) {
          const Q = G.segments.filter((he, N) => N !== O);
          t.exec('update-task', {
            id: f,
            task: { segments: Q },
          });
        }
      } else t.exec('delete-task', { id: f });
    }, [t, p, te, G, O]),
    ee = $(() => {
    ...
    ve = $(
      (f, Q, he) => (
        F && f.type === 'summary' && (f.unscheduled = !1),
        $t(f, t.getState(), Q),
        he || J(!1),
        f
      ),
    ...
    $e = $(
      (f) => {
        (f = {
          ...
          delete f.links,
          delete f.data,
          (U.indexOf('duration') === -1 || (f.segments && !f.duration)) &&
            delete f.duration;
        const Q = {
          id: p?.id || p,
          task: f,
    ...
    Ce = $(
      (f) => {
        ...
        if ((N && J(!0), (f.update = ve({ ...Q }, he, N)), !x)) le(f.update);
          ...
          const pe = j.find((Xe) => Xe.key === he),
            Ie = Q[he];
          (!pe.validation || pe.validation(Ie)) &&
            (!pe.required || Ie) &&
            $e(f.update);
        }
      },
...
const jn = ({ children: t, columns: n = null, api: s }) => {
  ...
  return (
    ...
    /* @__PURE__ */ a(tn, { api: l, columns: n, children: t })
  );
};
function Qn(t) {
  const { api: n, content: s, children: l } = t,
    o = ie(null),
    S = ie(null),
    [r, w] = ue({}),
    [x, y] = ue(null),
  ...
  function D(k) {
    for (; k; ) {
      if (k.getAttribute) {
        const G = k.getAttribute('data-tooltip-id'),
          p = k.getAttribute('data-tooltip-at'),
          F = k.getAttribute('data-tooltip');
  ...
  se(() => {
    ...
    if (k && d && (d.text || s)) {
      ...
      let p = !1,
        F = d.left,
        Z = d.top;
      G.right >= r.right && ((F = r.width - G.width - 5), (p = !0)),
        G.bottom >= r.bottom &&
          ((Z = d.top - (G.bottom - r.bottom + 2)), (p = !0)),
        p && L((re) => re && { ...re, left: F, top: Z });
    }
  }, [d, r, s]);
  const C = ie(null),
    Y = 300,
  ...
  function H(k) {
    ...
    c(() => {
      ...
      let M, g;
      Z === 'left'
        ? ((M = O.top + 5 - E.top), (g = O.right + 5 - E.left))
        : ((M = O.top + O.height - E.top), (g = re - E.left)),
        w(E),
        L({ top: M, left: g, text: p });
    });
  }
  function V(k) {
...
function Un({ fonts: t = !0, children: n }) {
  return n
    ? /* @__PURE__ */ a(ft, { fonts: t, children: n() })
    : /* @__PURE__ */ a(ft, { fonts: t });
}
function Zn({ fonts: t = !0, children: n }) {
  return n
    ? /* @__PURE__ */ a(ht, { fonts: t, children: n })
    : /* @__PURE__ */ a(ht, { fonts: t });
}
function Jn({ fonts: t = !0, children: n }) {
  return n
    ? /* @__PURE__ */ a(mt, { fonts: t, children: n })
    : /* @__PURE__ */ a(mt, { fonts: t });
}
export {
  qn as ContextMenu,
  Bn as Editor,
