...
import {
  prepareEditTask as Wt,
  grid as hn,
  extendDragOptions as mn,
  isSegmentMoveAllowed as gn,
  DataStore as wn,
  normalizeLinks as xn,
  normalizeZoom as pn,
  defaultColumns as yn,
  parseTaskDates as Mt,
  defaultTaskTypes as kn,
  getToolbarButtons as Dt,
  handleAction as _t,
...
import {
  ...
  defaultEditorItems as Ps,
  defaultMenuOptions as Hs,
  defaultTaskTypes as As,
  defaultToolbarButtons as Ws,
  getEditorItems as _s,
  getMenuOptions as zs,
  getToolbarButtons as Gs,
  registerScaleUnit as Us,
} from '@svar-ui/gantt-store';
import {
  useWritableProp as ut,
  useStore as O,
  useStoreWithCounter as nt,
  writable as Tn,
  useStoreLater as Ke,
...
function En(t, n) {
  ...
  function $(M) {
    if (k || p) return M.preventDefault(), !1;
  }
  function w(M) {
    M.which === 1 &&
      ((s = Ye(M)),
  ...
  function X(M) {
    ...
    if (i) {
      ...
      if (n && n.move && n.move({ id: o, top: Z, detail: r }) === !1) return;
      const fe = n.getTask(o),
        q = fe.$y;
      if (!C.start && C.y == q) return P();
      (C.start = !0), (C.y = fe.$y - 4), (i.style.top = Z + 'px');
      const ue = document.elementFromPoint(M.clientX, M.clientY),
        D = Ye(ue);
      if (D && D !== s) {
        const f = qe(D),
          G = D.getBoundingClientRect(),
          ne = G.top + G.height / 2,
          he = M.clientY + C.db > ne && D.nextElementSibling !== s,
          ie = M.clientY - C.dt < ne && D.previousElementSibling !== s;
        r?.after == f || r?.before == f
          ? (r = null)
          : he
            ? (r = { id: o, after: f })
            : ie && (r = { id: o, before: f });
  ...
  function _(M) {
    k
      ? (M.preventDefault(), X(M.touches[0]))
  ...
  return /* @__PURE__ */ Pe('div', {
    ...
    children: [
      ...
      /* @__PURE__ */ u('div', {
        ...
        children: i ? /* @__PURE__ */ u(i, { row: t, column: n }) : t.text,
      }),
    ],
  });
}
...
function Nn(t) {
  const {
    ...
    [ue, D] = pe(null),
    f = v(
      () => (!P || !b ? [] : C && r ? P : P.slice(b.start, b.end)),
      [P, b, C, r],
    ),
    G = N(
      (l, y) => {
  ...
  const De = re(null),
    ...
    be = v(() => (C && r ? 0 : (b?.from ?? 0)), [b, C, r]),
    te = v(() => M?.height ?? 0, [M]),
    x = v(
      () => (!s && o !== 'grid' ? (k ?? 0) > (i ?? 0) : (k ?? 0) > (de ?? 0)),
      [s, o, k, i, de],
    ),
    H = v(() => {
      const l = {};
    ...
    U = v(() => {
      ...
      const y = l.findIndex((z) => z.id === 'text'),
        E = l.findIndex((z) => z.id === 'add-task');
      if (
        (y !== -1 && (l[y].cell && (l[y]._cell = l[y].cell), (l[y].cell = Sn)),
        E !== -1)
      ) {
        l[E].cell = l[E].cell || Lt;
        const z = l[E].header;
        if (
          (typeof z != 'object' && (l[E].header = { text: z }),
          (l[E].header.cell = z.cell || Lt),
          n)
        )
          l.splice(E, 1);
        else if (s) {
    ...
    T = N(() => U.some((l) => l.flexgrow && !l.hidden), []),
    J = v(() => T(), [T, Ee]),
    me = v(() => {
      ...
      if (!J) {
        ...
        if (U.some((xe) => xe.$width)) {
          ...
          (E = U.reduce(
            (ye, Ne) => (
              Ne.hidden || ((xe += Ne.width), (ye += Ne.$width || Ne.width)), ye
            ),
            0,
          )),
            xe > E && E > y && (z = !0);
          ...
          return (
            ...
            l.map(
              (ye) => (
                ye.id !== 'add-task' &&
                  !ye.hidden &&
                  (ye.$width || (ye.$width = ye.width),
                  (ye.width = ye.$width * xe)),
                ye
              ),
            )
          );
    ...
    ge = N(
      (l) => {
        if (!T()) {
          const y = me.reduce(
            (E, z) => (
              l && z.$width && (z.$width = z.width),
              E + (z.hidden ? 0 : z.width)
            ),
            0,
          );
          y !== k && p(y);
    ...
    m = N(() => {
      U.filter((y) => y.flexgrow && !y.hidden).length === 1 &&
        U.forEach((y) => {
    ...
    ce = N(
      (l) => {
        if (!n) {
          const y = Ve(l),
            E = dn(l, 'data-col-id');
          !(E && U.find((xe) => xe.id == E))?.editor &&
            y &&
            d.exec('show-editor', { id: y });
        }
      },
      [d, n],
      // cols is defined later; relies on latest value at call time
    ),
    we = v(() => (Array.isArray(B) ? B.map((l) => l.id) : []), [B]),
    Y = re(be);
  (Y.current = be),
    ae(() => {
      const l = (E) => {
    ...
    ae(() => {
      if (!_ || !V) return;
      const { id: l } = _,
        y = V.getState().focusCell;
      y &&
        y.row !== l &&
        he.current &&
        he.current.contains(document.activeElement) &&
        V.exec('focus-cell', {
          row: l,
          column: y.column,
        });
  ...
  Ae(),
    ae(() => {
      Ae();
    }, [F, ve, oe, d, m, ge, f, Z, fe, q, h]);
  const _e = N((l) => {
    F(l),
      l.intercept('hotkey', (y) => Re.current.handleHotkey(y)),
      l.intercept('scroll', () => !1),
      l.intercept('select-row', () => !1),
      l.intercept('sort-rows', (y) => {
        const E = Re.current.sortVal,
          { key: z, add: xe } = y,
          ye = E ? E.find((ze) => ze.key === z) : null;
        let Ne = 'asc';
        return (
          ye && (Ne = !ye || ye.order === 'asc' ? 'desc' : 'asc'),
          d.exec('sort-tasks', {
            key: z,
            order: Ne,
            add: xe,
      ...
      l.intercept('update-cell', (y) => {
        const { id: E, column: z, value: xe } = y,
          ye = Re.current.tasks.find((Ne) => Ne.id === E);
        if (ye) {
          const Ne = { ...ye };
          let ze = xe;
...
function Pn(t) {
  ...
  function o(L) {
    ...
    do {
      const V = C[k];
      k === L &&
        (r > h ? (p = 0) : (p = Math.min((h - r) / V.duration, 1) * 100)),
        (r += V.duration),
        k++;
...
function _n(t) {
  const {
    ...
    q = v(() => w.lengthUnitWidth, [w]),
    ue = v(() => w.lengthUnit || 'day', [w]),
    D = re(!1),
    [f, G] = pe(void 0),
    [ne, he] = pe(null),
    ie = re(null),
  ...
  const me = re(200),
    ...
    ce = v(() => m && W[W.length - 1]?.id, [m, W]);
  ae(() => {
    if (B && m && B) {
      const { id: e } = B,
        c = ge.current?.querySelector(`.wx-bar[data-id='${e}']`);
  ...
  const we = N(() => {
    ...
    Ce = N(
      (e, c, a) => {
        if (
          ...
          (a || (a = r.getTask(qe(e))),
          a.type === 'milestone' || a.type === 'summary')
        )
          return '';
        const g = Ye(c, 'data-segment');
        g && (e = g);
        const { left: S, width: R } = e.getBoundingClientRect(),
          I = (c.clientX - S) / R;
    ...
    Re = N(
      (e) => {
        ...
        const a = Math.min(e.startX, e.currentX),
          g = Math.max(e.startX, e.currentX),
          S = Math.min(e.startY, e.currentY),
          R = Math.max(e.startY, e.currentY);
        return k.filter((I) => {
          const A = I.$x,
            ee = I.$x + I.$w,
            Se = $e.get(I.id) ?? I.$y,
    ...
    xe = N(() => {
      ...
      if (e) {
        ...
        e.ctrlKey
          ...
          : (W.length > 0 && r.exec('select-task', { id: null, marquee: !0 }),
            c.forEach((a, g) => {
              r.exec('select-task', {
                id: a.id,
                toggle: g > 0,
      ...
      if (j) {
        const { dx: c, originalPositions: a } = j,
          g = Math.round(c / q);
        if (g !== 0) {
          let S = !0;
          a.forEach((R, I) => {
      ...
      if (ie.current) {
        ...
        (ie.current = null),
          ...
          g.classList.remove('wx-progress-in-drag'),
          (D.current = !0),
          Y();
      } else if (ne) {
        const {
          ...
          if (!Me)
            ...
            const Oe = 1440 * 60 * 1e3,
              Ie =
                ...
                Oe;
            a === 'move'
              ? ((Se.start = new Date(We.start.getTime() + Ie)),
                (Se.end = new Date(We.end.getTime() + Ie)))
              : a === 'start'
                ? ((Se.start = new Date(We.start.getTime() + Ie)),
                  (Se.end = We.end))
                : a === 'end' &&
    ...
    }, [r, Y, ne, q, ue, te, j, Re, W]),
    ye = N(
      (e, c) => {
        const { clientX: a, clientY: g } = c,
          S = ge.current;
        if (S) {
        ...
        if (!n) {
          if (te) {
            ...
            x((Me) => ({
              ...
              H.current &&
                ((H.current.currentX = A), (H.current.currentY = ee));
            return;
          }
          if (j) {
            const R = a - j.startX;
          ...
          if (ie.current) {
            const { node: R, x: I, id: A } = ie.current,
              ee = (ie.current.dx = a - I),
              Me = Math.round((ee / R.offsetWidth) * 100);
            let Se = ie.current.progress + Me;
            (ie.current.value = Se = Math.min(Math.max(0, Se), 100)),
              r.exec('update-task', {
                id: A,
                task: { progress: Se },
                inProgress: !0,
            ...
            const R = Ye(e);
            if (R) {
              const I = r.getTask(qe(R)),
                ee = Ye(e, 'data-segment') || R,
                Me = Ce(ee, c, I);
  ...
  const Xt = N(
    ...
    et = N(
      (e, c) => {
        ...
        return e === a
          ? !0
          : !!V.find(
              (S) => S.target == e && S.source == a && S.type === Je(g, c),
            );
      },
      [f, F, Je],
    ),
    ...
    ht = N(
      (e, c, a) => {
        ...
        R.setUTCDate(R.getUTCDate() + A),
          ...
          c.forEach((ee, Me) => {
            const Se = `task-${Date.now()}-${Me}`,
              We = An(R, ee._startCellOffset || 0, w),
              Oe = new Date(We.getTime() + (ee._startDayOfWeek || 0) * g);
            Oe.setUTCHours(0, 0, 0, 0);
            const Ue = new Date(Oe.getTime() + (ee._durationDays || 7) * g);
            Ue.setUTCHours(0, 0, 0, 0),
              console.log(
                '[paste] task:',
                ee.text,
                'newStart:',
    ...
    Ft = N(
      (e) => {
        ...
        if (c) {
          const a = r.getTask(c),
            g = k.find((R) => R.id === c);
          console.log('[click] task:', a?.text, 'id:', c),
            console.log('[click] api.getTask:', {
              start: a?.start,
              end: a?.end,
              duration: a?.duration,
            }),
    ...
    Bt = N(
      (e) => {
        if (Ee || De.current) return e.preventDefault(), !1;
      },
      [Ee],
    ),
    mt = v(() => d.map((e) => e.id), [d]),
    ...
    lt = N((e) => b && P.byId(e).$critical, [b, P]),
    xt = N(
      (e) => {
        if (M?.auto) {
          const c = P.getSummaryId(e, !0),
            a = P.getSummaryId(f.id, !0);
          return (
            f?.id &&
            !(Array.isArray(c) ? c : [c]).includes(f.id) &&
    ...
    pt = N(() => {
        ...
        a = e
          .map((A) => {
            ...
            const {
              ...
              kt =
                Me.end && Me.start
                  ? Math.round((Me.end.getTime() - Me.start.getTime()) / c)
                  : 0,
              bt = Me.start ? (Me.start.getUTCDay() + 6) % 7 : 0;
            return (
              console.log(
                '[copy] task:',
                ee.text,
      ...
      if (!a.length) return;
      const S = a[0].parent,
        R = a.filter((A) => A.parent === S);
      if (R.length === 0) return;
      const I = R.reduce(
        (A, ee) => (ee.start && (!A || ee.start < A) ? ee.start : A),
        null,
      );
      (ct = R.map((A) => ({
        ...A,
  ...
  ae(
    () =>
      h
        ? r.intercept('hotkey', (c) => {
            if (c.key === 'ctrl+c' || c.key === 'meta+c') return pt(), !1;
    ...
    ae(() => {
      ...
      const e = (c) => {
        c.key === 'Escape' &&
          (c.preventDefault(), c.stopPropagation(), T(null));
      };
      return (
  ...
  const yt = v(() => {
    if (!te) return null;
    const e = Math.min(te.startX, te.currentX),
      c = Math.min(te.startY, te.currentY),
      a = Math.abs(te.currentX - te.startX),
      g = Math.abs(te.currentY - te.startY);
    return {
      left: `${e}px`,
      top: `${c}px`,
      width: `${a}px`,
    ...
    children: [
      ...
      fe.map((e) => {
        ...
        const c = se.has(e.id),
          a =
            `wx-bar wx-${gt(e.type)}` +
            (Ee && ne && e.id === ne.id ? ' wx-touch' : '') +
            (f && f.id === e.id ? ' wx-selected' : '') +
            (Ae.has(e.id) ? ' wx-selected' : '') +
            (lt(e.id) ? ' wx-critical' : '') +
            (e.$reorder ? ' wx-reorder-task' : '') +
            (Q && e.segments ? ' wx-split' : '') +
            (c ? ' wx-collision' : ''),
          g =
            'wx-link wx-left' +
            (f ? ' wx-visible' : '') +
            (!f || (!et(e.id, !0) && xt(e.id)) ? ' wx-target' : '') +
            (f && f.id === e.id && f.start ? ' wx-selected' : '') +
            (lt(e.id) ? ' wx-critical' : ''),
          S =
            'wx-link wx-right' +
            (f ? ' wx-visible' : '') +
            (!f || (!et(e.id, !1) && xt(e.id)) ? ' wx-target' : '') +
            (f && f.id === e.id && !f.start ? ' wx-selected' : '') +
            (lt(e.id) ? ' wx-critical' : '');
        return /* @__PURE__ */ Pe(
          Jt,
      ...
      le &&
        ...
        le.tasks.map((e, c) => {
          const g =
              (Math.floor(le.currentX / q) + (e._startCellOffset || 0)) * q,
            S = e._originalWidth || q,
            R = e._originalHeight || Z,
            I = ve.get(e.row) ?? (e.$y || 0);
          return /* @__PURE__ */ u(
            'div',
            {
      ...
      /* @__PURE__ */ u(
        'div',
        {
          ...
          children: (o.cells || []).map((h, C) => {
            const r = n ? n(h.date, h.unit) : '',
              k = 'wx-cell ' + (h.css || '') + ' ' + (r || ''),
              p = typeof h.value == 'string' && h.value.includes('<');
            return /* @__PURE__ */ u(
              'div',
              {
                ...
                ...(p
                  ? { dangerouslySetInnerHTML: { __html: h.value } }
                  : { children: h.value }),
              },
              C,
            );
...
function On(t) {
  ...
  function Te() {
    const x = q.current,
      j = Math.ceil((Z || 0) / (_ || 1)) + 1,
      K = Math.floor(((x && x.scrollTop) || 0) / (_ || 1)),
      U = Math.max(0, K - f),
      ke = K + j + f,
      le = U * (_ || 0);
    w.exec('render-data', {
  ...
  const Ee = N(
    (x) => {
      ...
      const K = q.current;
      if (!K) return;
      const { clientWidth: U } = K,
        ke = w.getTask(H);
      if (ke.$x + ke.$w < K.scrollLeft)
        w.exec('scroll-chart', { left: ke.$x - (B || 0) }),
          (K.scrollLeft = ke.$x - (B || 0));
      else if (ke.$x >= U + K.scrollLeft) {
        const le = U < ke.$w ? B || 0 : ke.$w;
        w.exec('scroll-chart', { left: ke.$x - U + le }),
          (K.scrollLeft = ke.$x - U + le);
      }
    },
    [w, B],
  );
  ...
  function Le(x) {
    if (Q && (x.ctrlKey || x.metaKey)) {
      x.preventDefault();
      const H = q.current,
        j = x.clientX - (H ? H.getBoundingClientRect().left : 0);
  ...
  return (
    ...
    ae(() => {
      ...
      const x = q.current,
        { clientWidth: H } = x,
        j = /* @__PURE__ */ new Date(),
        K = b.rows[b.rows.length - 1]?.cells;
      if (!K) return;
      let U = -1,
        ke = 0;
      const le = [];
      for (let J = 0; J < K.length; J++) {
        ...
        if (me.unit === 'week') {
          ...
          me.unit === 'day' &&
            j.getUTCFullYear() === ge.getUTCFullYear() &&
            j.getUTCMonth() === ge.getUTCMonth() &&
            j.getUTCDate() === ge.getUTCDate() &&
            (U = J);
        ke += me.width;
      }
      let T = U;
      if ((U > 0 && (T = U - 1), T >= 0 && le[T])) {
        const J = le[T],
          me = Math.max(0, J.left);
        (x.scrollLeft = me),
          w.exec('scroll-chart', { left: me }),
          (D.current = !0);
      }
    }, [V, b, Z, w]),
    Un('chart'),
...
function Yn(t) {
  ...
  function w(ie) {
    ...
    if (i != 'x') for (let Ee in Te) Te[Ee] = Te[Ee].reverse();
    return Te;
  }
  const [d, X] = pe(!1),
    [W, _] = pe(null),
  ...
  const oe = N(
      (ie) => {
        ...
        V(de);
        let Te;
        de <= r ? (Te = 'chart') : C - de <= k ? (Te = 'grid') : (Te = 'all'),
          M.current !== Te && ($(Te), (M.current = Te)),
          P.current && clearTimeout(P.current),
          (P.current = setTimeout(() => o && o(de), 100));
      },
      [C, r, k, o],
    ),
    Z = N(() => {
  ...
  function D(ie) {
    if (h) {
      ...
      $(de), L(de);
    } else if (F === 'grid' || F === 'chart') ue(), L('all');
    else {
...
function Kn(t) {
      ...
      (T) => {
        M(
          (J) => (
            T !== J &&
              (T
                ? ((he.current = G), G === 'all' && ne('grid'))
                : (!he.current || he.current === 'all') && ne('all')),
            T
          ),
        );
      },
  ...
  const de = v(() => {
    let T;
    return (
      W.every((J) => J.width && !J.flexgrow)
        ? (T = W.reduce((J, me) => J + parseInt(me.width), 0))
        : P && G === 'chart'
          ? (T = parseInt(W.find((J) => J.id === 'action')?.width) || 50)
          : (T = 440),
      (Q = T),
      T
    );
  ...
  const Te = v(() => (Z ?? 0) - (D ?? 0), [Z, D]),
    Ee = v(() => d.width, [d]),
    Le = v(() => {
      if (!h || !b) return w.length * X;
  ...
  ae(() => {
    let T;
    return (
      be.current && ((T = new ResizeObserver(te)), T.observe(be.current)),
      () => {
        T && T.disconnect();
      }
    );
  ...
  const U = N(
    (T) => {
      ...
      if (!ce) return;
      const { id: Y } = T,
        Ce = J.findIndex((se) => se.id === Y);
      if (Ce > -1) {
        const se = we - me.height,
          $e = Ce * ge,
          ve = ce.scrollTop;
        let Re = null;
        $e < ve ? (Re = $e) : $e + ge > ve + se && (Re = $e - se + ge + m),
          Re !== null &&
            ($.exec('scroll-chart', { top: Math.max(Re, 0) }),
            (x.current.scrollTop = Math.max(Re, 0)));
      }
    },
    [$],
  );
...
const Qn = (t) =>
  ...
  Ts = Ht(function (
    ...
    const be = re();
    be.current = De;
    const te = v(() => new wn(Tn), []),
      x = v(() => ({ ...dt, ...ot }), []),
      H = Ge(Xe.i18n),
      j = v(() => (H ? H.extend(x, !0) : rt(x)), [H, x]),
      K = v(() => j.getRaw().calendar, [j]),
      U = v(() => {
        let se = {
          zoom: jn(B, K),
          scales: Ot(r, K),
        ...
        return (
          se.zoom &&
            (se = {
              ...se,
              ...pn(se.zoom, Vn(K, j.getGroup('gantt')), se.scales, w),
            }),
          se
        );
      }, [B, r, k, C, w, K, j]),
        ...
        const se = /* @__PURE__ */ new Map(),
          ...
          ve = (Re) => {
            Re.forEach((Ae) => {
              ...
              $e.set(Ae.id, _e),
                se.has(_e) || se.set(_e, []),
                se.get(_e).push(Ae.id),
                Ae.data && Ae.data.length > 0 && ve(Ae.data);
            });
          };
    ...
    const ce = v(
      () => ({
        ...
        getTable: (se) =>
          se
            ? new Promise(($e) => setTimeout(() => $e(m.current), 1))
            : m.current,
    ...
    const Y = v(() => {
        const se = /* @__PURE__ */ new Date(),
          $e = K?.weekStart ?? 0,
          ve = new Date(
            Date.UTC(se.getUTCFullYear(), se.getUTCMonth(), se.getUTCDate()),
          ),
          Ae = (ve.getUTCDay() - $e + 7) % 7;
      ...
      Ce = v(
        () => (se, $e) => {
          let ve = [];
          if (D)
            $e == 'day' && !D.getDayHours(se) && ve.push('wx-weekend'),
              $e == 'hour' && !D.getDayHours(se) && ve.push('wx-weekend');
          else if (P) {
            const Re = P(se, $e);
            Re && ve.push(Re);
          }
    ...
    return /* @__PURE__ */ u(Xe.i18n.Provider, {
      value: j,
      children: /* @__PURE__ */ u(Fe.Provider, {
        value: ce,
  ...
  const s = Ge(Xe.i18n),
    ...
    p = v(() => {
      ...
      return (
        ...
        let d = { ...w, disabled: !1 };
        return (
          (d.handler = zt(F, d.id) ? (X) => _t(t, X.id, null, o) : d.handler),
          d.text && (d.text = o(d.text)),
          d.menuText && (d.menuText = o(d.menuText)),
          d
        );
      });
    }, [n, t, o, h, r]),
        ...
        p.forEach(($) => {
          ...
          if (w === 'add-task') F.push($);
            ...
            F.push({
              ...$,
              disabled:
                $.isDisabled && L.some((d) => $.isDisabled(d, t.getState())),
            });
          }
        }),
        F.filter(($, w) => {
...
const Cs = Ht(function (
  ...
  function b(D) {
    return D.map(
      (f) => (
        (f = { ...f }),
        f.text && (f.text = w(f.text)),
        f.subtext && (f.subtext = w(f.subtext)),
        f.data && (f.data = b(f.data)),
        f
      ),
    );
  }
  function P() {
    const D = n.length ? n : Rt({ splitTasks: _ }),
      f = D.find((G) => G.id === 'convert-task');
    return (
      f &&
        ((f.data = []),
        (d || []).forEach((G) => {
  ...
  const M = v(() => P(), [s, n, d, _, w]),
    ...
    oe = N(
      (D, f) => {
        ...
        if (G) {
          ...
          ne !== null
            ? (V.current = { id: G.id, segmentIndex: ne })
            : (V.current = G.id),
            (!Array.isArray(X) || !X.includes(G.id)) &&
              s &&
              s.exec &&
              s.exec('select-task', { id: G.id });
        }
        return G;
      },
      [s, i, X],
    ...
    Z = N(
      (D) => {
        const f = D.action;
        f && (zt(B, f.id) && _t(s, f.id, V.current, w), C && C(D));
      },
      [s, w, C, B],
    ),
    fe = N(
      (D, f) => {
        ...
        if (
          ne &&
          (D.isHidden &&
            (ne = !G.some((he) => D.isHidden(he, s.getState(), V.current))),
          D.isDisabled)
        ) {
          const he = G.some((ie) => D.isDisabled(ie, s.getState(), V.current));
          D.disabled = he;
        }
        return ne;
      },
...
function Jn({ api: t, autoSave: n, onLinksChange: s }) {
  const o = Ge(Xe.i18n).getGroup('gantt'),
    L = O(t, 'activeTask'),
    h = O(t, '_activeTask'),
    C = O(t, '_links'),
    r = O(t, 'schedule'),
    k = O(t, 'unscheduledTasks'),
    [p, V] = pe();
  function F() {
    if (L) {
      const X = C.filter((_) => _.target == L).map((_) => ({
          link: _,
          task: t.getTask(_.source),
        })),
        W = C.filter((_) => _.source == L).map((_) => ({
          link: _,
          task: t.getTask(_.target),
        }));
      return [
...
function es(t) {
  const { value: n, time: s, format: i, onchange: o, onChange: L, ...h } = t,
    C = L ?? o;
  function r(k) {
    const p = new Date(k.value);
    p.setUTCHours(n.getUTCHours()),
      p.setUTCMinutes(n.getUTCMinutes()),
      C && C({ value: p });
  }
  return /* @__PURE__ */ Pe('div', {
...
function $s({
  ...
  const P = O(t, '_activeTask'),
    M = O(t, 'activeTask'),
    Q = O(t, 'unscheduledTasks'),
    oe = O(t, 'links'),
    Z = O(t, 'splitTasks'),
    fe = v(() => Z && M?.segmentIndex, [Z, M]),
    q = v(() => fe || fe === 0, [fe]),
    ue = v(() => bn({ unscheduledTasks: Q }), [Q]),
    D = O(t, 'undo'),
    [f, G] = pe({}),
    [ne, he] = pe(null),
    [ie, de] = pe(),
    ...
    De = v(() => {
      ...
      if ((q && P.segments ? (m = { ...P.segments[fe] }) : (m = { ...P }), o)) {
        ...
        return (
          ue.forEach(({ key: we, comp: Y }) => {
            if (Y !== 'links') {
              const Ce = m[we];
              Y === 'date' && Ce instanceof Date
                ? (ce[we] = d(Ce))
                : Y === 'slider' && we === 'progress'
                  ? (ce[we] = `${Ce}%`)
                  : (ce[we] = Ce);
            }
          }),
  ...
  function He(m, ce) {
    return m.map((we) => {
      const Y = { ...we };
      if (
        (we.config && (Y.config = { ...Y.config }),
        Y.comp === 'links' &&
          t &&
          ((Y.api = t), (Y.autoSave = r), (Y.onLinksChange = x)),
        Y.comp === 'select' && Y.key === 'type')
      ) {
      ...
      return (
        Y.comp === 'slider' &&
          Y.key === 'progress' &&
          (Y.labelTemplate = (Ce) => `${$(Y.label)} ${Ce}%`),
        Y.label && (Y.label = $(Y.label)),
        Y.config?.placeholder &&
          (Y.config.placeholder = $(Y.config.placeholder)),
        ce &&
          (Y.isDisabled && Y.isDisabled(ce, t.getState())
            ? (Y.disabled = !0)
            : delete Y.disabled),
        Y
      );
  ...
  const be = v(() => {
      ...
      return (
        (m = He(m, ie)),
        ie
          ? m.filter((ce) => !ce.isHidden || !ce.isHidden(ie, t.getState()))
          : m
      );
    }, [n, ue, ie, Le, $, t, r]),
    te = v(() => be.map((m) => m.key), [be]);
  ...
  const H = N(() => {
    ...
    j = N(() => {
      ...
      if (q) {
        if (P?.segments) {
          const ce = P.segments.filter((we, Y) => Y !== fe);
          t.exec('update-task', {
            id: m,
            task: { segments: ce },
          });
    ...
    ke = N(
      (m, ce, we) => (
        Q && m.type === 'summary' && (m.unscheduled = !1),
        Wt(m, t.getState(), ce),
        we || he(!1),
        m
      ),
    ...
    le = N(
      (m) => {
        (m = {
          ...
          delete m.links,
          delete m.data,
          (te.indexOf('duration') === -1 || (m.segments && !m.duration)) &&
            delete m.duration;
        const ce = {
          id: M?.id || M,
          task: m,
    ...
    T = N(
      (m) => {
        ...
        if ((Y && he(!0), (m.update = ke({ ...ce }, we, Y)), !r)) de(m.update);
          ...
          const Ce = be.find((ve) => ve.key === we),
            se = ce[we];
          (!Ce.validation || Ce.validation(se)) &&
            (!Ce.required || se) &&
            le(m.update);
        }
      },
...
const Ms = ({ children: t, columns: n = null, api: s }) => {
  ...
  return (
    ...
    /* @__PURE__ */ u(Cn, { api: i, columns: n, children: t })
  );
};
function Ds(t) {
  ...
  function F(b) {
    for (; b; ) {
      if (b.getAttribute) {
        const P = b.getAttribute('data-tooltip-id'),
          M = b.getAttribute('data-tooltip-at'),
          Q = b.getAttribute('data-tooltip');
  ...
  ae(() => {
    ...
    if (b && p && (p.text || s)) {
      ...
      let M = !1,
        Q = p.left,
        oe = p.top;
      P.right >= h.right && ((Q = h.width - P.width - 5), (M = !0)),
        P.bottom >= h.bottom &&
          ((oe = p.top - (P.bottom - h.bottom + 2)), (M = !0)),
        M && V((Z) => Z && { ...Z, left: Q, top: oe });
    }
  }, [p, h, s]);
  const $ = re(null),
    w = 300,
  ...
  function X(b) {
    ...
    d(() => {
      ...
      let D, f;
      oe === 'left'
        ? ((D = fe.top + 5 - ue.top), (f = fe.right + 5 - ue.left))
        : ((D = fe.top + fe.height - ue.top), (f = Z - ue.left)),
        C(ue),
        V({ top: D, left: f, text: M });
    });
  }
  function W(b) {
...
function Rs({ fonts: t = !0, children: n }) {
  return n
    ? /* @__PURE__ */ u(vt, { fonts: t, children: n() })
    : /* @__PURE__ */ u(vt, { fonts: t });
}
function Es({ fonts: t = !0, children: n }) {
  return n
    ? /* @__PURE__ */ u(Ct, { fonts: t, children: n })
    : /* @__PURE__ */ u(Ct, { fonts: t });
}
